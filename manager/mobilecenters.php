    ...
    if(isset($_SESSION['Username']) && $_SESSION['AdminLevel'] == 0){
        ...
        if ($do == 'Manage') {          // Mange Page 
            ...
            $stmt = $con->prepare(" SELECT  BloodCenter.id, BloodCenter.name, BloodCenter.address,
                                            ...
                                            Distirct.name AS district
                                    FROM    BloodCenter, Country, Distirct
                                    WHERE   BloodCenter.bloodCenterTypeid = 2 &&
                                            BloodCenter.countryid = Country.id &&
                                            BloodCenter.districtid = Distirct.id $query
                                 ");
            $stmt->execute();
            $rows = $stmt->fetchAll();
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row" id="main">
                    <!-- Page Heading -->
                    <div class="go-title">
                        <div class="pull-right"><a href="mobilecenters.php?do=Add"><div class="btn btn-primary"><i class="fa fa-plus"></i> Add New Center</div></a></div>
                                ...
                                <?php
                                    foreach($rows as $row) {
                                        echo '<tr>';
                                            ...
                                            echo '<td>  
                                                <a href="mobilecenters.php?do=Edit&centerid='. $row['id'] .'" style="padding:5px" class="btn btn-success"><i class="fa fa-edit"></i> Edit</a>
                                                <a href="mobilecenters.php?do=Delete&centerid='. $row['id'] .'" style="padding:5px" class="btn btn-danger confirm"><i class="fa fa-close"></i> Delete</a> ';
                                                
                                                if($row['registrationApproved'] == 0) {
                                                    echo '<a href="mobilecenters.php?do=Activate&centerid='. $row['id'] .'" style="padding:5px" class="btn btn-info"><i class="fa fa-close"></i> Activate</a>';
                                                }
...
<?php
        ...
        } elseif($do == 'Add') {        // Add New Mobile Center Page   
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if ($check == 1) {
                    ...
                    $stmt = $con->prepare(" INSERT INTO `BloodCenter`
                                            ...
                                            VALUES 
                                            (:name, :address, :from, :to, :country, 2, :district, 1);
                                        ");
                    $stmt->execute(array(
                        'name'      =>  $name,
                        'address'   =>  $address,
